...
LevelDesigner::LevelDesigner(const std::string &level_name, int width, int height)
    ...
    : graphics{"Level Designer", 1280, 720}, tilemap{width, height}, level{level_name},
      ...
      dt{0.1}, performance_frequency{SDL_GetPerformanceFrequency()}, prev_counter{SDL_GetPerformanceCounter()},
      lag{0.0},

      display_rect{0.0f, 0.0f, graphics.width * (2.0f / 3.0f), static_cast<float>(graphics.height)},

      tiles_rect{
          graphics.width * (2.0f / 3.0f), 0.0f, graphics.width * (1.0f / 3.0f), static_cast<float>(graphics.height)
      } {
    update_title();
...
void LevelDesigner::handle_event(SDL_Event *event) {
    ...
    if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN && event->button.button == SDL_BUTTON_LEFT) {
        ...
        if (mouse_x > display_rect.x && mouse_y > display_rect.y && mouse_x < display_rect.x + display_rect.w && mouse_y
        ...
        else if (mouse_x > tiles_rect.x && mouse_y > tiles_rect.y && mouse_x < tiles_rect.x + tiles_rect.w && mouse_y <
                 tiles_rect.y + tiles_rect.h) {
...
void LevelDesigner::input() {
    ...
    if (keys[SDL_SCANCODE_S] && (keys[SDL_SCANCODE_LCTRL] || keys[SDL_SCANCODE_RCTRL] || keys[SDL_SCANCODE_LGUI] || keys
                                 [SDL_SCANCODE_RGUI])) {
        save();
    }
    ...
    for (int y = 0; y < VISIBLE_MAP_HEIGHT; ++y) {
        for (int x = 0; x < VISIBLE_MAP_WIDTH; ++x) {
            ...
            if (tilemap_x >= 0 && tilemap_x < tilemap.width && tilemap_y >= 0 && tilemap_y < tilemap.height) {
                ...
                Color color = selected_tile == Vec<int>{tilemap_x, tilemap_y}
                                  ? Color{255, 255, 0, 255}
                                  : Color{0, 0, 0, 255};
...
void LevelDesigner::place_enemy(std::string enemy_name) {
    level.enemy_locations[{static_cast<float>(selected_tile.x),
        static_cast<float>(selected_tile.y)}] = enemy_name;
